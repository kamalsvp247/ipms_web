...
it('renders the public payment-helper landing without auth', function () {
    $this->get('/payment-helper')
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->where('app.name', 'BLITZ IVAC Payment Helper')
            ->has('downloadUrl'));
});

it('serves the extension zip for download when the build exists', function () {
    Storage::fake('public');
    Storage::disk('public')->put('extensions/blitz-payment-helper.zip', 'PK-fake-zip-bytes');

    $response = $this->get('/payment-helper/download');

    $response->assertOk();
    expect($response->headers->get('content-type'))->toContain('application/zip');
    expect($response->headers->get('content-disposition'))->toContain('blitz-payment-helper.zip');
});
